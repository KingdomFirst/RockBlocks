...
// * Added campus context to attempt to set pledge to child account of block setting financial account based on campus match.
// </notice>
//
using System;
...
namespace RockWeb.Plugins.rocks_kfs.Finance
{
    ...
    [DisplayName( "Pledge Entry with Campus Context" )]
    [Category( "KFS > Finance" )]
    [Description( "Allows a website visitor to create pledge for the configured accounts, start and end date. This block also creates a new person record if a matching person could not be found. This block has campus context to attempt to set pledge to child account of block setting financial account based on campus match." )]
