...
// * Adds ability to reprint labels
// * Adds ability to move people
// * Adds ability to checkout (rather than delete)
// </notice>
//
using System;
    ...
    public partial class Locations : Rock.Web.UI.RockBlock
    {
        ...
        protected override void OnLoad( EventArgs e )
        {
            ...
            if ( campus != null )
            {
                ...
                if ( !Page.IsPostBack )
                {
                    ...
                    {
                        CurrentNavPath = Session["CurrentNavPath"] as string;
                    }
        ...
        protected void rptNavItems_ItemDataBound( object sender, RepeaterItemEventArgs e )
        {
            ...
            if ( navItem != null )
            {
                ...
                {
                    // if user can't edit and the occurrence is closed, show warning
                    int? groupId = null;
                ...
                if ( tgl != null )
                {
                    if ( loc != null && ( UserCanAdministrate || IsUserAuthorized( "CloseLocations" ) ) )
                    {
            ...
            if ( tgl != null )
            {
                ...
                if ( _useKFSCloseAttribute && groupId.HasValue )
                {
                    using ( var rockContext = new RockContext() )
                    {
                        ...
                        if ( activeScheduleIds.Count > occurrences.Count )
                        {
                            ...
                            foreach ( var scheduleId in activeScheduleIds )
                            {
                                if ( !occurrenceScheduleIds.Contains( scheduleId ) )
                                {
                                    ...
                                    {
                                        occurrence.SetAttributeValue( "rocks.kfs.OccurrenceClosed", ( !tgl.Checked ).ToString() );
                                        occurrence.SaveAttributeValue( "rocks.kfs.OccurrenceClosed" );
                                        rockContext.SaveChanges();
                                        Rock.CheckIn.KioskDevice.Clear();
                                    }
                                }
                        ...
                        // set the location active if the occurrence is turning active
                        var location = new LocationService( rockContext ).Get( id.Value );
...
",
        ...
        private void BuildNavigationControls()
        {
            if ( NavData != null )
            {
                ...
                SaveViewState();
                Session["CurrentNavPath"] = CurrentNavPath;
                ...
                if ( item != null )
                {
                    ...
                    if ( locationItem != null && locationItem.HasGroups )
                    {
                        if ( UserCanAdministrate || IsUserAuthorized( "CloseLocations" ) )
                        {
                            ...
                            {
                                string groupItemKey = ( numParts > 2 ) ? pathParts[numParts - 2] : string.Empty;
                                int? currentGroupId = groupItemKey.Length > 1 ? groupItemKey.Substring( 1 ).AsIntegerOrNull() : null;
                                ...
                                using ( var rockContextOccurrence = new RockContext() )
                                {
                                    ...
                                    var occurrences = new AttendanceOccurrenceService( rockContextOccurrence )
                                                            ...
                                                            .Where( o =>
                                                                ...
                                                                ( o.ScheduleId.HasValue && activeScheduleIds.Contains( o.ScheduleId.Value ) ) )
                                                            .WhereAttributeValue( rockContextOccurrence, "rocks.kfs.OccurrenceClosed", "True" )
                                                            .ToList();
                        ...
                        else if ( _useKFSCloseAttribute && !( UserCanAdministrate || IsUserAuthorized( "CloseLocations" ) ) && !locationItem.IsActive )
                        {
                            lblLocationWarningHeader.Visible = !locationItem.IsActive;
                        }
                        else if ( _useKFSCloseAttribute && !( UserCanAdministrate || IsUserAuthorized( "CloseLocations" ) ) )
                        {
                            // if user can't edit and the occurrence is closed, show warning
                            int? groupId = null;
                            if ( pathParts.Length >= 3 && pathParts[2].StartsWith( "G" ) )
                            ...
                            if ( groupId.HasValue )
                            {
                                ...
                                {
                                    // check the location active first
                                    lblLocationWarningHeader.Visible = !( new LocationService( newRockContext ).GetNoTracking( locationItem.Id ).IsActive );

                                    // if we see the location is active, then check the occurrence attribute
                                    if ( !lblLocationWarningHeader.Visible )
                                    {
                                        var activeScheduleIds = new List<int>();
                                        var schedulesWithCheckin = new ScheduleService( newRockContext )
                                            .Queryable().AsNoTracking()
                                            .Where( s => s.CheckInStartOffsetMinutes.HasValue );
                                        foreach ( var schedule in schedulesWithCheckin )
                                        {
                                        ...
                                        var occurrences = new AttendanceOccurrenceService( newRockContext )
                                                                ...
                                                                .Where( o =>
                                                                    ...
                                                                    o.LocationId == locationItem.Id &&
                                                                    ( o.ScheduleId.HasValue && activeScheduleIds.Contains( o.ScheduleId.Value ) ) )
                                                                .WhereAttributeValue( newRockContext, "rocks.kfs.OccurrenceClosed", "True" )
                                                                .ToList();
                            ...
                            using ( var rockContextLocationMode = new RockContext() )
                            {
                                ...
                                var groupIds = new AttendanceOccurrenceService( rockContextLocationMode )
                                                        ...
                                                        .Where( o =>
                                                            ...
                                                            o.GroupId.HasValue &&
                                                            ( o.ScheduleId.HasValue && activeScheduleIds.Contains( o.ScheduleId.Value ) ) )
                                                        .WhereAttributeValue( rockContextLocationMode, "rocks.kfs.OccurrenceClosed", "True" )
                                                        .Select( o => o.GroupId.Value )
                                                        .Distinct()
                                                        .ToList();
                    ...
                    else
                    {
                        ...
                        lblLocationWarningHeader.Visible = false;
                    }
                }
                else
                {
