...
", btnCopyToClipboard.ClientID );
        ...
        /// <summary>
        /// Handles the RowSelected event of the gGiversGifts control.
        /// </summary>
        /// <param name="sender">The source of the event.</param>
        ...
        /// <summary>
        /// Handles the SelectedIndexChanged event of the ddlAccountType control.
        /// </summary>
        /// <param name="sender">The source of the event.</param>
        /// <param name="e">The <see cref="EventArgs"/> instance containing the event data.</param>
        ...
        private void BuildDynamicControls()
        {
            ...
            if ( _campusAccounts == null )
            {
                ...
                {
                    _campusAccounts = new Dictionary<int, List<Tuple<int, string, int?>>>();
                    foreach ( var campusAccounts in new FinancialAccountService( rockContext )
                        ...
                        {
                            CampusId = c.Key,
                            Accounts = c.OrderBy( a => a.Order )
                                .ThenBy( a => a.Name )
                                .Select( a => new { a.Id, a.Name, a.AccountTypeValueId } )
                                .ToList()
                        } ) )
                    {
                        _campusAccounts.Add( campusAccounts.CampusId, new List<Tuple<int, string, int?>>() );
                        foreach ( var account in campusAccounts.Accounts )
                        {
                            _campusAccounts[campusAccounts.CampusId].Add( new Tuple<int, string, int?>( account.Id, account.Name, account.AccountTypeValueId ) );
                        }
                    }
                }
            }
        ...
        public void LoadDropDowns()
        {
            ...
            ddlAccountType.Items.Insert( 0, Rock.Constants.None.ListItem );
        }
