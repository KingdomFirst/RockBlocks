...
namespace RockWeb.Blocks.KFS.Utility
{
    ...
    /// 
    /// </summary>
    [DisplayName( "Merge Template Entry - Finance" )]
    [Category( "KFS > Utility" )]
    [Description( "Used for merging finance data into output documents, such as Word, Html, using a pre-defined template." )]
    public partial class MergeTemplateEntryFinance : RockBlock
    {
        ...
        protected void btnMerge_Click( object sender, EventArgs e )
        {
			...
			parameters.Add( "@entityId" , hfEntitySetId.Value.AsInteger());
			var ds = DbService.GetDataSet( "[dbo].[sp_CustKfs_BatchAccountTransactionDate]", System.Data.CommandType.StoredProcedure, parameters, 300 );
			var myData = ds.Tables[0].AsEnumerable()
			.Select( r => new  {
				Id = r.Field<int>("Id"),
				AccountName = r.Field<string>("AccountName"),
				GlCode = r.Field<string>("GlCode"),
				TransactionDays = ds.Tables[0].AsEnumerable()
					.Select( t => new  {
						Id = t.Field<int>("Id"),
						TransactionDate = t.Field<DateTime>("TransactionDate"),
						AccountTransactionDateTotal = t.Field<decimal>("AccountTransactionDateTotal")
					})
					.Where( t => t.Id == r.Field<int>("Id") )
					.ToJson()
			})
			.Distinct();
			
			mergeFields.Add( "GivingByAccountAndDate", myData );
