/****** Object:  StoredProcedure [dbo].[com_kfs_spPersonImport_CSV]    Script Date: 1/4/2019 5:09:20 PM ******/
SET ANSI_NULLS ON
GO
SET QUOTED_IDENTIFIER ON
GO

CREATE PROCEDURE [dbo].[com_kfs_spPersonImport_CSV]
...
Installation:
- CREATE PROCEDURE [dbo].[com_kfs_spPersonImport_CSV] AS ;
...
DECLARE @BatchName NVARCHAR(250) = 'Crowdrise'
DECLARE @AccountId INT = 98;
...
IF (EXISTS (SELECT * 
                 ...
                 AND  TABLE_NAME = '_com_kfs_peopleCsvTemp'))
BEGIN
    DROP TABLE [_com_kfs_peopleCsvTemp];
END

CREATE TABLE _com_kfs_peopleCsvTemp (
...
')
INSERT _com_kfs_peopleCsvTemp (FirstName, LastName, Email, ConnectionStatusId, GroupId, ForeignGuid)
SELECT 
...
;WITH personData AS (
    SELECT * FROM _com_kfs_peopleCsvTemp
), RockMatch AS (
