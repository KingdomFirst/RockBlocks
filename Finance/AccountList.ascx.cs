...
namespace RockWeb.Blocks.Finance
{
    ...
    public partial class AccountList : RockBlock
    {
        ...
        protected void rAccountFilter_DisplayFilterValue( object sender, GridFilter.DisplayFilterValueArgs e )
        {
            switch ( e.Key )
            {
                ...
                case "Account Type":
                    ...
                    var accountTypeId = e.Value.AsIntegerOrNull();
                    if ( accountTypeId.HasValue )
                    {
                        var accountType = DefinedValueCache.Read( accountTypeId.Value );
                        if ( accountType != null )
                        {
                            e.Value = accountType.Value;
                        }
                    }
        ...
        protected void rAccountFilter_ApplyFilterClick( object sender, EventArgs e )
        {
            ...
            rAccountFilter.SaveUserPreference( "Account Type", ddlAccountType.SelectedValue );
            BindGrid();
        }
        ...
        private IQueryable<FinancialAccount> GetAccounts( RockContext rockContext )
        {
            ...
            var accountTypeId = int.MinValue;
            if ( int.TryParse( rAccountFilter.GetUserPreference( "Account Type" ), out accountTypeId ) )
            {
                accountQuery = accountQuery.Where( account => account.AccountTypeValueId == accountTypeId );
            }
        ...
        private void BindFilter()
        {
            ...
            var accountTypeCache = DefinedTypeCache.Read( Rock.SystemGuid.DefinedType.FINANCIAL_ACCOUNT_TYPE.AsGuid() );
            ddlAccountType.BindToDefinedType( accountTypeCache, true );
            ...
            ddlAccountType.SelectedValue = rAccountFilter.GetUserPreference( "Account Type" );
        }
