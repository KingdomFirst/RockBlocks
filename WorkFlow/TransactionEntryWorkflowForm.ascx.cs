...
using rocks.kfs.Workflow.Action.Finance;

namespace RockWeb.Plugins.rocks_kfs.WorkFlow
{
    #region Block Attributes

    [DisplayName( "Transaction Entry Workflow Form" )]
    [Category( "KFS > WorkFlow" )]
    [Description( "Block used to create transactions from a workflow." )]

    #endregion

    #region Block Settings

    [LinkedPage( "Confirmation Page", "An alternate confirmation page to display after a transaction is successfully processed. Leaving this blank will keep the user on the transaction entry confirmation screen.", false, order: 0 )]
    ...
    [SystemCommunicationField( "Confirm Account", "Confirm Account Communication Template", false, Rock.SystemGuid.SystemCommunication.SECURITY_CONFIRM_ACCOUNT, "Email Templates", 13, "ConfirmAccountTemplate" )]
...
", "Text Options", 24 )]
    ...
    [SystemCommunicationField( "Receipt Email", "The system communication to use to send the receipt.", false, "", "Email Templates", 28 )]
...
TransactionAccountDetails: [
  ...
  },
  {
    ...
    public partial class TransactionEntryWorkflowForm : RockBlock
    {
        #region Fields
        ...
        private Dictionary<string, string> _pageParams;
        private FinancialAccount _account;
        private decimal? _amount;
        private string _transactionIdAttributeKey;
        private int _transactionId;
        private bool _useSavedPaymentAccounts;
        ...
        /// <summary>
        /// Gets or sets the workflow person.
        /// </summary>
        /// <value>
        /// The workflow person.
        /// </value>
        protected Person WorkflowPerson
        {
            get { return ViewState["WorkflowPerson"] as Person; }
            set { ViewState["WorkflowPerson"] = value; }
        }
        ...
        protected override void OnInit( EventArgs e )
        {
            ...
            var guid = PageParameter( "WorkflowGuid" ).AsGuidOrNull();
            if ( guid.HasValue )
            {
                _workflowService = new WorkflowService( _rockContext );
                _workflow = _workflowService.Get( guid.Value );
                if ( _workflow != null )
                {
                    _workflow.LoadAttributes();
                    _pageParams = new Dictionary<string, string>();
                    _pageParams.Add( "WorkflowId", _workflow.Id.ToString() );
                }
            }
        ...
        protected void GetWorkflowValues()
        {
            if ( _workflow != null )
            {
                ...
                foreach ( var activity in _workflow.ActiveActivities )
                {
                    ...
                    {
                        if ( action.ActionType.EntityType.Name.Equals( "rocks.kfs.Workflow.Action.Finance.TransactionEntryForm", StringComparison.CurrentCultureIgnoreCase ) )
                        {
                            actions.Add( action );
                        }
                    }
                ...
                if ( actions.Any() )
                {
                    ...
                    {
                        _action = actions[0];
                        _action.LoadAttributes();

                        var attributeValues = _action.ActionTypeCache.AttributeValues;

                        var financialAccountService = new FinancialAccountService( rockContext );
                        var accountAttributeGuid = attributeValues[TransactionEntryForm.ACCOUNT_KEY].Value.AsGuid();
                        var accountGuid = _action.GetWorkflowAttributeValue( accountAttributeGuid ).AsGuid();
                        _account = financialAccountService.Get( accountGuid );

                        var attributeValueService = new AttributeValueService( rockContext );
                        var amountAttributeGuid = attributeValues[TransactionEntryForm.AMOUNT_KEY].Value.AsGuid();
                        _amount = attributeValueService.Queryable().FirstOrDefault( v => v.Attribute.Guid == amountAttributeGuid && v.EntityId == _workflow.Id ).ValueAsNumeric;
                        var useSavedPaymentAccountsAttributeGuid = attributeValues[TransactionEntryForm.USE_SAVED_PAYMENT_ACCOUNTS_KEY].Value.AsGuid();
                        if ( !useSavedPaymentAccountsAttributeGuid.IsEmpty() )
                        {
                            var attributeValue = attributeValueService.Queryable().FirstOrDefault( v => v.Attribute.Guid == useSavedPaymentAccountsAttributeGuid && v.EntityId == _workflow.Id );
                            if ( attributeValue != null )
                            {
                                _useSavedPaymentAccounts = attributeValue.Value.AsBoolean( false );
                            }
                        }
                        else
                        {
                            _useSavedPaymentAccounts = false;
                        }

                        var personAliasService = new PersonAliasService( rockContext );
                        var personAliasAttributeGuid = attributeValues[TransactionEntryForm.PERSON_ATTRIBUTE_KEY].Value.AsGuid();
                        var personAliasGuid = _action.GetWorkflowAttributeValue( personAliasAttributeGuid ).AsGuid();
                        WorkflowPerson = personAliasService.Get( personAliasGuid ).Person;

                        var transactionIdAttributeGuid = attributeValues[TransactionEntryForm.TRANSACTION_ID_KEY].Value.AsGuid();
                        _transactionIdAttributeKey = AttributeCache.Get( transactionIdAttributeGuid ).Key;
                    }
                }
            }
        }
        ...
        protected void lbSaveAccount_Click( object sender, EventArgs e )
        {
            ...
            using ( var rockContext = new RockContext() )
            {
                // reset because workflow person viewstate isn't loaded during init
                Guid personGuid;
                if ( Guid.TryParse( hfWorkflowPerson.Value, out personGuid ) )
                {
                    _targetPerson = new PersonService( new RockContext() ).Get( personGuid );
                }
        ...
        private IEntity GetTransactionEntity()
        {
            ...
            if ( transactionEntityTypeGuid.HasValue )
            {
                ...
                if ( transactionEntityType != null )
                {
                    ...
                    var entityGuid = this.PageParameter( this.GetAttributeValue( "EntityIdParam" ) ).AsGuidOrNull();
                    ...
                    else if ( entityGuid.HasValue )
                    {
                        ...
                        if ( serviceInstance != null )
                        {
                            ...
                            transactionEntity = getMethod.Invoke( serviceInstance, new object[] { entityGuid.Value } ) as Rock.Data.IEntity;
                        }
                    }
                }
            }
        ...
        {
            if ( component != null && _useSavedPaymentAccounts )
            {
        ...
        private void BindSavedAccounts( RockContext rockContext, bool oneTime )
        {
            ...
            if ( _targetPerson != null && _useSavedPaymentAccounts )
            {
        ...
        private void SetControlOptions()
        {
            ...
            if ( GetAttributeValue( "EnableBusinessGiving" ).AsBoolean() && WorkflowPerson == null )
            {
        ...
        private void GetAccounts()
        {
            ...
            else if ( _account != null && _amount.HasValue )
            {
                var accountItem = new AccountItem( _account.Id, _account.Order, _account.Name, _account.CampusId, _account.PublicName, _amount.Value, false );
                SelectedAccounts.Add( accountItem );
            }
            else
            {
        ...
        {
            bool givingAsBusiness = GetAttributeValue( "EnableBusinessGiving" ).AsBoolean() && !tglGiveAsOption.Checked && WorkflowPerson == null;
            bool userLoggedIn = ( WorkflowPerson == null && CurrentPerson != null );
        ...
        private void ShowSuccess( GatewayComponent gatewayComponent, Person person, PaymentInfo paymentInfo, PaymentSchedule schedule, FinancialPaymentDetail paymentDetail, RockContext rockContext )
        {
            ...
            if ( !( paymentInfo is ReferencePaymentInfo ) && !string.IsNullOrWhiteSpace( TransactionCode ) && gatewayComponent.SupportsSavedAccount( paymentInfo.CurrencyTypeValue ) && _useSavedPaymentAccounts )
            {
                ...
                hfWorkflowPerson.Value = WorkflowPerson.Guid.ToString();
            }
            else
            {
            ...
            if ( _workflow != null && _action != null )
            {
                if ( _transactionIdAttributeKey != null )
                {
                    _workflow.SetAttributeValue( _transactionIdAttributeKey, _transactionId );
                    _workflow.SaveAttributeValue( _transactionIdAttributeKey );
                }
                ...
                NavigateToLinkedPage( "ConfirmationPage", _pageParams );
            }
        }
