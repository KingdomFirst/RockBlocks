...
namespace RockWeb.Plugins.com_kfs.Finance
{
    ...
    public partial class PledgeList : RockBlock
    {
        ...
        {
            var currentPerson = RockPage.GetCurrentContext( EntityTypeCache.Read<Person>() ) as Person;
            var ascf = new Dictionary<string, string>();
            ascf.Add( "pledgeId", "0" );
            ascf.Add( "personId", currentPerson.Id.ToString() );
            NavigateToLinkedPage( "DetailPage", ascf );
        }
