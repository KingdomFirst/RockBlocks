...
// * Adds option to sort the location results by sort order if using KFS load balance.
// * The core pull request provides block setting to sort by sort order no matter the custom attribute.
// * This block should be obsolete once v9 is released.
// </notice>
//
using System;
            ...
            else
            {
                if ( !Page.IsPostBack )
                {
                    ...
                    if ( availLocations.Any() )
                    {
                        ...
                        {
                            if ( group.Group.GetAttributeValue( "com.kfs.LoadBalanceLocations" ).AsBoolean() )
                            {
